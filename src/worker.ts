...
export function solveChallenge(challenge: string, salt: string, algorithm: string = 'SHA-256', max: number = 1e6, start: number = 0 ): { promise: Promise<Solution | null>; controller: AbortController } {
    ...
    const fn = async () => {
        for (let n = start; n <= max; n += 1) {
            ...
            const t = hashHex(algorithm as Algorithm, salt + n);
            if (t === challenge) {
                return {
                    number: n,
...
export function hashHex( algorithm: Algorithm, data: ArrayBuffer | string ) {
...
export function hash(algorithm: Algorithm, data: ArrayBuffer | string) {
